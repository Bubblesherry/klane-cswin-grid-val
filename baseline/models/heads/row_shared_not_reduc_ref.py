    ...
    def get_lane_exist_and_cls_wise_maps(self, label_tensor, is_one_hot=False, is_ret_list=True):
        ...
        if is_ret_list:
            ...
            for idx_cls in range(self.num_cls):
                ...
                ext = ret_exist[:, idx_cls, :, :] if b == 1 else np.squeeze(ret_exist[:, idx_cls, :, :])
                cls = ret_maps[:, idx_cls, :, :] if b == 1 else np.squeeze(ret_maps[:, idx_cls, :, :])
                list_ext.append(torch.tensor(ext).cuda())
                list_cls.append(torch.tensor(cls).cuda())
